...
const ACCOUNT_TYPES = [
  ['BANK', 'Bank Account'], ['CASH', 'Cash'], ['CREDIT_CARD', 'Credit Card'], ['WALLET', 'Wallet'],
  ['UPI', 'UPI'], ['INVESTMENT', 'Investment'], ['OTHER', 'Other'],
];
const EMPTY_FORM = { name: '', type: 'BANK', institution: '', currency: 'INR', openingBalance: '0' };
const formatCurrency = (value, currency = 'INR') => new Intl.NumberFormat('en-IN', { style: 'currency', currency, minimumFractionDigits: 2 }).format(Number(value || 0));
...
function Accounts() {
  ...
  const loadData = async () => {
    try {
      ...
      const response = await financeApi.accounts.list();
  ...
  const totalsByCurrency = accounts.reduce((result, account) => {
    const currency = account.currency || 'INR';
    result[currency] = (result[currency] || 0) + Number(account.currentBalance || 0);
    return result;
  }, {});
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingId) await financeApi.accounts.update(editingId, formData); else await financeApi.accounts.create(formData);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.summaryGrid}>
        ...
        <div style={styles.summaryCard}><span style={styles.label}>Combined Balance</span><div style={styles.currencyTotals}>{Object.entries(totalsByCurrency).map(([currency, total]) => <strong key={currency} style={total >= 0 ? styles.income : styles.expense}>{formatCurrency(total, currency)}</strong>)}</div></div>
      </div>
