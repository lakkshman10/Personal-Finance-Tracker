...
const normalizeAlertPercent = (value = 80) => {
  ...
  if (!Number.isFinite(percent) || percent < 0 || percent > 100) {
    throw new Error('Alert percent must be between 0 and 100.');
  }
...
const budgetService = {
  async list(userId, month) {
    return budgetRepository.findByUserId(userId, {
      month: month ? normalizeMonth(month) : undefined,
    });
  },
  ...
  async create(userId, input = {}) {
    ...
    return budgetRepository.create({
      ...
      alertPercent: normalizeAlertPercent(alertPercent),
      duration,
  ...
  async update(userId, id, input = {}) {
    ...
    const nextAlertPercent = input.alertPercent !== undefined
      ? normalizeAlertPercent(input.alertPercent)
      : existing.alertPercent.toString();
    ...
    const amountChanged = nextAmount !== existing.amount.toString();
    const alertChanged = nextAlertPercent !== existing.alertPercent.toString();
    const categoryChanged = nextCategoryId !== existing.categoryId;
    const monthChanged = nextMonth.getTime() !== existing.month.getTime();

    if (amountChanged || alertChanged || categoryChanged || monthChanged || nextDuration !== existing.duration) {
      if (existing.adjustments.length >= MAX_ADJUSTMENTS) {
        throw new Error('This budget has already been adjusted once. Further adjustments are not allowed.');
      }
      if (!input.reason || typeof input.reason !== 'string' || !input.reason.trim()) {
        throw new Error('A reason is required when adjusting a budget.');
      }
      if (input.reason.trim().length > 500) throw new Error('Adjustment reason must be 500 characters or fewer.');

      const updated = await require('../config/prisma').$transaction(async (tx) => {
        ...
        await tx.budgetAdjustment.create({
          data: {
            ...
            oldAlertPercent: existing.alertPercent,
            newAlertPercent: nextAlertPercent,
      ...
      return budgetRepository.findByIdForUser(updated.id, userId);
    }
